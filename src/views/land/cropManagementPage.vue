      ...
      const onEachFeature = (feature, layer) => {
        layer.on({
          click: () => {
            ...
            getCropLandById(feature.properties.id)
              .then((result) => {
                // 关闭加载中的Toast消息
                closeToast();
                // 检查获取地块信息的结果代码
                if (result.code !== 0) {
                  ...
                  showFailToast(result.msg);
                  console.error("获取地块作物变更信息失败：", result.msg);
                  return;
                }
                if (result.data.length > 0) {
                  popupContentData = result.data[0];
                }
                const popupContent = `
      <div style="display:flex;flex-direction: column;align-items: flex-start; width: 140px;">
                      <p style="margin:0 0 5px 0;"><strong>土地类型:</strong> ${landName}</p>
                      <p style="margin:0 0 5px 0;"><strong>种植作物:</strong> ${
                        popupContentData
                          ? popupContentData.cropName
                          : "暂无信息"
                      }</p>
                      <p style="margin:0 0 5px 0;"><strong>种植年份:</strong> ${
                        popupContentData
                          ? popupContentData.leaseYear
                          : "暂无信息"
                      }</p>
                      <p style="margin:0 0 5px 0;"><strong>种植人:</strong> ${
                        popupContentData
                          ? popupContentData.landLessee
                          : "暂无信息"
                      }</p>
                    </div>
            `;
